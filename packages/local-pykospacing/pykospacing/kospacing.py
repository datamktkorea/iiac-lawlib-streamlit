...
이 모듈은 TensorFlow 모델을 사용하여 한국어 문장의 띄어쓰기를 교정하는 Spacing 클래스를 정의합니다.
"""
...
try:
    ...
    class TFSMLayer(tf.keras.layers.Layer):
        """TensorFlow SavedModel을 로드하고 실행하기 위한 레이어."""
        ...
        def call(self, inputs):
            return self.model.signatures[self.call_endpoint](inputs)
...
# 현재 파일(kospacing.py)의 디렉토리 경로를 구합니다.
package_dir = os.path.dirname(os.path.abspath(__file__))

model_path = os.path.join(package_dir, "resources", "models")
dic_path = os.path.join(package_dir, "resources", "dicts", "c2v.dic")
...
class Spacing:
    """입력 문자열에 대한 띄어쓰기를 예측하고 교정하는 클래스."""

    def __init__(self, rules=[]):
        """Spacing 클래스를 초기화합니다.

        Args:
            rules (list, optional): 사용자 정의 띄어쓰기 규칙 리스트. Defaults to [].
        """
        self._model = MODEL
        self._w2idx = W2IDX
    ...
    def __call__(
        ...
        for i in range(len(splitted_sent)):
            ...
            spaced_sent = self.get_spaced_sent(
                filtered_sent, deleted_str_list, deleted_idx_list, orig_sent, post_process
            )
